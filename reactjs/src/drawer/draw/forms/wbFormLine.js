...
import BrowserDetect from '../../BrowserDetect'
 ...
 // 'use strict';
	...
	moveLink = function (owner,action){
		...
		owner.y=b.y;
			...
			owner.points=[];
			owner.points.push(new Point(a.x-b.x,a.y-b.y));
	...
	drawPerm= function (shape,init) {
		...
		try{
			...
			console.log(err);
			throw new Error("[E] No function matches:"+this.type+"Perm() in FormLine (wbFormLine.js)");
	...
	straight =  function (owner,fx,fy){
		...
		owner.points=[];
		MC.setStrokeStyle(5);
		var d= Math.sqrt( (lc.x)*lc.x + (lc.y)*lc.y );
		// var r = d%owner.segSize;
		var tot =d/owner.segSize;
		var lastX=0;
		var lastY=0;
		for (var i=0;i<tot; ++i){
			var x=lc.x*(i/tot);	
	...
	straightPerm = function(owner,shape,init){
		var MC =owner.bg.graphics;
	...
	freePerm = function (owner,shape,init){
		var MC =owner.bg.graphics;
		var HTC =owner.bg.hitArea.graphics;
		//console.log(owner);
		var tot = owner.points.length;
		...
		*/
		...
		owner.x+=negX;
		owner.y+=negY;
		owner.rect= new Rectangle(0,0,Math.abs(negX)+maxX,Math.abs(negY)+maxY);
	...
	bezierPerm = function (owner,shape,init){//http://blog.sklambert.com/finding-the-control-points-of-a-bezier-curve/
		var tot = owner.points.length;
		var strokeIn=5;
		var MC =owner.bg.graphics;
		var HTC =owner.bg.hitArea.graphics;
	...
	setDimension = function (owner,w,h,cx,cy){
	...
	EventDispatcher.initialize(FormLine.prototype);
	// var p = extend(FormLine, Container);

	
	function drawArrow(owner){
		owner.rotary=false;
		owner.elastic=false;
		owner.arrow  = new Shape();
		owner.arrow.snapToPixel=true;
		owner.ball  = new Shape();
	...
	export default promote(FormLine, "Container");